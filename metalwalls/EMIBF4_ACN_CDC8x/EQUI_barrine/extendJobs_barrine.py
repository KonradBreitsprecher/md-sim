...
for subdir, dirs, files in os.walk(rootdir):
	if len(dirs) == 0:
		...
		print "#PBS -q workq"
		print "#PBS -A uq-ChemEng"
		print "#PBS -m ae"
		print "#PBS -M r.burt@uq.edu.au"
		print "#PBS -e error_" + job
		print "#PBS -o output_" + job
		print "#PBS -N " + job
		print "#PBS -l select=" + nodes + ":ncpus=" + ppn + ":mpiprocs=" + ppn + ":NodeType=medium"
		print "#PBS -l walltime=" + walltime.zfill(2) + ":00:00"
                print ""
		print "source /usr/share/modules/init/bash"
		print "module load intel-fc-13 openmpi/1.5.3-intel"
		print ""
		print "INPUTDIR=" + ap
		print "JOB=\"" + job + "\""                                                           
		print "mkdir -p $HOME/metalwalls_bench/$JOB"
		print "cd $HOME/metalwalls_bench/$JOB"
		...
		print ""
		print "mpirun -np " + str(cores) + "  /work1/uqrburt/metalwalls/bin/metalwalls.exe"
