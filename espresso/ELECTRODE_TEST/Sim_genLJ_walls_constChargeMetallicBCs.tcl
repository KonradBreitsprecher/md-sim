...
set obs_energy [open "$path/obs_energy${charge_dens}.dat" "w"]
...
if {$useCheckpoint == "yes"} {
	...
	set cfile [open "$pathToCheckpoint" "r"]
	while { [blockfile $cfile read auto] != "eof" } {}
	close $cfile
	integrate 0
	...
	inter coulomb elc 1e-4 $gap dielectric-contrasts -1 -1
	
} else {

	inter coulomb $l_b p3m tunev2 accuracy 1e-4
	inter coulomb elc 1e-4 $gap
	...
	set cfile [open "$path/startConfig_scharge_${charge_dens}.dat" w]
	blockfile $cfile write particles "pos" $anionlist
	blockfile $cfile write particles "pos quat" $cationComlist
	blockfile $cfile write particles "pos" $cationlist
	close $cfile
...
#Write config
set cfile [open "$path/checkpoint_scharge_${charge_dens}.dat" w]
blockfile $cfile write particles "pos v f" $anionlist
blockfile $cfile write particles "pos quat v f" $cationComlist
blockfile $cfile write particles "pos v f" $cationlist
close $cfile
...
set out [ open "$path/orientation_hist_scharge_${charge_dens}" "w" ]
...
set out [open "$path/charge_dens_scharge_${charge_dens}" "w"]
...
set out [open "$path/total_dens_scharge_${charge_dens}" "w"]
...
set out [open "$path/anion_dens_scharge_${charge_dens}" "w"]
...
set out [open "$path/cation_dens_com_scharge_${charge_dens}" "w"]
...
set out [open "$path/cation_dens_c1_scharge_${charge_dens}" "w"]
...
set out [open "$path/cation_dens_c2_scharge_${charge_dens}" "w"]
...
set out [open "$path/cation_dens_c3_scharge_${charge_dens}" "w"]
	foreach z $zlist c $cation_den_c3 e $err { puts $out "$z $c $e" }
close $out

#Tot pot
#set out [open "$path/tot_potential_${charge_dens}" "w"]
#set pot ""
...
set out [open "$path/sim_params_scharge_${charge_dens}" "w"]
puts $out "Path:             $path"
