...
#ElcGap
set gap [expr 0.20*$box_l]

setmd box_l $box_h $box_h [expr $box_l+$gap]

#Get dielectric boundaries at inner Layers -> shift everything  
set elc_shift [expr -2.0*$wall_layer_dist]
...
for {set i 0} { $i < $n_ion } {incr i} {
    ...
    set posz [expr $elc_shift + ($box_l-3.0*$wall_layer_dist-2*$lj_sig_w)*[t_random]+2.5*$wall_layer_dist+$lj_sig_w]
...
if { $wall_bool == 1 } {
#------------------------
puts "->placing wall atoms"; flush stdout
#------------------------
# first wall first layer
set count $j
set mod 0
set stepx 1.43
set d 1.43

for { set x1 0 } { $x1<$box_h } { set x1 [expr $x1+$stepx] } {
	for { set y1 0 } { $y1<$box_h } { set y1 [expr $y1+2*$d] } {
		set posx $x1
		set posy $y1
		set posz $elc_shift
		part $count pos $posx $posy $posz type $wall_type fix 1 1 1 mass 12
...
set mod 0
set stepx 1.43
for { set x2 [expr 0.5*$d] } { $x2<$box_h } { set x2 [expr $x2+$stepx] } {
	for { set y2 [expr $d] } { $y2<$box_h } { set y2 [expr $y2+2*$d] } {
		set posx $x2
		set posy $y2
		set posz $elc_shift
		part $count pos $posx $posy $posz type $wall_type fix 1 1 1 mass 12
...
set mod 0
# first wall second layer
set stepx 1.43
for { set x1 1.43 } { $x1<$box_h } { set x1 [expr $x1+$stepx] } {
	for { set y1 0 } { $y1<$box_h } { set y1 [expr $y1+2*1.43] } {
		set posx $x1
		set posy $y1
		set posz [expr $elc_shift + 3.35]
		part $count pos $posx $posy $posz type $wall_type fix 1 1 1 mass 12
		set count [expr $count+1]
		incr mod
		if { [expr $mod] %2 !=0 } { 
			set stepx [expr 2*1.43]
		} else { 
			set stepx [expr 1.43] 
		}
	}
...
set mod 0
set stepx 1.43
for { set x2 [expr 1.5*$d] } { $x2<$box_h } { set x2 [expr $x2+$stepx] } {
	for { set y2 [expr $d] } { $y2<$box_h } { set y2 [expr $y2+2*$d] } {
		set posx $x2
		set posy $y2
		set posz [expr $elc_shift + 3.35]
...
for { set x1 0 } { $x1<$box_h } { set x1 [expr $x1+$stepx] } {
	for { set y1 0 } { $y1<$box_h } { set y1 [expr $y1+2*$d] } {
		set posx $x1
		set posy $y1
		set posz [expr $elc_shift + $box_l+2*3.35]
		part $count pos $posx $posy $posz type $wall_type fix 1 1 1 mass 12
		set count [expr $count+1]
		incr mod 
		if { [expr $mod] %2 !=0 } { 
...
set mod 0
# second wall second layer
set stepx 1.43
for { set x1 1.43 } { $x1<$box_h } { set x1 [expr $x1+$stepx] } {
	for { set y1 0 } { $y1<$box_h } { set y1 [expr $y1+2*1.43] } {
		set posx $x1
		set posy $y1
		set posz [expr $elc_shift + $box_l+3*3.35]
		part $count pos $posx $posy $posz type $wall_type fix 1 1 1 mass 12
		set count [expr $count+1]
		incr mod
		if { [expr $mod] %2 !=0 } { 
...
for { set x1 0 } { $x1<$box_h } { set x1 [expr $x1+$stepx] } {
	for { set y1 0 } { $y1<$box_h } { set y1 [expr $y1+2*$d] } {
		set posx $x1
		set posy $y1
		set posz [expr $elc_shift + $box_l+4*3.35]
		part $count pos $posx $posy $posz type $wall_type fix 1 1 1 mass 12
		set count [expr $count+1]
		incr mod 
		if { [expr $mod] %2 !=0 } { 
...
for {set cut 4.0} {$cut > 0.00001} {set cut [expr $cut*0.75]} {
	...
	inter $c1_type $wall_type lennard-jones $lj_eps_mix_c1_w $lj_sig_mix_c1_w [expr 2.5*$lj_sig_mix_c1_w] 0.004 0 [expr $cut*$lj_sig_mix_c1_w]
	...
	inter $c2_type $wall_type lennard-jones $lj_eps_mix_c2_w $lj_sig_mix_c2_w [expr 2.5*$lj_sig_mix_c2_w] 0.004 0 [expr $cut*$lj_sig_mix_c2_w]
	...
	inter $c3_type $wall_type lennard-jones $lj_eps_mix_c3_w $lj_sig_mix_c3_w [expr 2.5*$lj_sig_mix_c3_w] 0.004 0 [expr $cut*$lj_sig_mix_c3_w]
	inter $a_type $a_type lennard-jones $lj_eps_a $lj_sig_a [expr 2.5*$lj_sig_a] 0.004 0 [expr $cut*$lj_sig_a]
	inter $a_type $wall_type lennard-jones $lj_eps_mix_a_w $lj_sig_mix_a_w [expr 2.5*$lj_sig_mix_a_w] 0.004 0 [expr $cut*$lj_sig_mix_a_w]
...
inter $c1_type $wall_type lennard-jones $lj_eps_mix_c1_w $lj_sig_mix_c1_w [expr 2.5*$lj_sig_mix_c1_w] 0.004 0 0
...
inter $c2_type $wall_type lennard-jones $lj_eps_mix_c2_w $lj_sig_mix_c2_w [expr 2.5*$lj_sig_mix_c2_w] 0.004 0 0
...
inter $c3_type $wall_type lennard-jones $lj_eps_mix_c3_w $lj_sig_mix_c3_w [expr 2.5*$lj_sig_mix_c3_w] 0.004 0 0
inter $a_type $a_type lennard-jones $lj_eps_a $lj_sig_a [expr 2.5*$lj_sig_a] 0.004 0 0
inter $a_type $wall_type lennard-jones $lj_eps_mix_a_w $lj_sig_mix_a_w [expr 2.5*$lj_sig_mix_a_w] 0.004 0 0 
...
set obs_energy [open "$path/obs_energy_${charge_dens}.dat" "w"]
...
set obs_orientation [observable new molecule_orientation ids $oriList minx 0 maxx $box_h xbins 1 miny 0 maxy $box_h ybins 1 minz 0 maxz $box_l zbins $nbins]
set obs_anion_pos_id [observable new density_profile type [list $a_type] minx 0 maxx $box_h xbins 1 miny 0 maxy $box_h ybins 1 minz 0 maxz $box_l zbins $nbins]
set obs_cation_pos_id [observable new density_profile type [list $non_virtual_type] minx 0 maxx $box_h xbins 1 miny 0 maxy $box_h ybins 1 minz 0 maxz $box_l zbins $nbins]
set obs_cation_c1_id [observable new density_profile type [list $c1_type] minx 0 maxx $box_h xbins 1 maxy $box_h ybins 1 minz 0 maxz $box_l zbins $nbins]
set obs_cation_c2_id [observable new density_profile type [list $c2_type] minx 0 maxx $box_h xbins 1 maxy $box_h ybins 1 minz 0 maxz $box_l zbins $nbins]
set obs_cation_c3_id [observable new density_profile type [list $c3_type] minx 0 maxx $box_h xbins 1 maxy $box_h ybins 1 minz 0 maxz $box_l zbins $nbins]
...
#set vtf_file [open "$path/trajectory/trajectory.vtf" "w"]
...
set out [ open "$path/orientation_hist_${charge_dens}" "w" ]
...
set out [open "$path/charge_dens_${charge_dens}" "w"]
...
set out [open "$path/anions_force_${charge_dens}" "w"]
...
set out [open "$path/cation_dens_com_sigma_${charge_dens}" "w"]
...
set out [open "$path/cation_dens_c1_sigma_${charge_dens}" "w"]
...
set out [open "$path/cation_dens_c2_sigma_${charge_dens}" "w"]
...
set out [open "$path/cation_dens_c3_sigma_${charge_dens}" "w"]
