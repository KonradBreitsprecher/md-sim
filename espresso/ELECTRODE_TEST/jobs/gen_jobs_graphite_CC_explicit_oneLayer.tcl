...
set pot_max 6.0
set num_pots 10.0
set path "$::env(HOME)/git/sim_scripts/jobs/pbsjobs_graphite_CC_explicit_oneLayer"
puts $path
for {set run 1} { $run <= $runs } {incr run} {
	set pbsfname "$path/job_gen_run${run}.pbs"
	set o [open $pbsfname "w"]
	puts $o "#!/bin/bash"
	puts $o "#PBS -N BMIMPF6_Graphite_CCexplicit_oneLayer_run${run}"
...
#		puts $o "aprun -n 4 -N 4 \$HOME/git/espresso_git/build/Espresso \$HOME/git/sim_scripts/Sim_graphite_walls_explict.tcl $sc \$HOME/data/graphite_CCexplicit/${run} \$HOME/data/graphite_CCexplicit/${run}/checkpoint_scharge_${sc}.dat &"
		puts $o "aprun -n 4 -N 4 \$HOME/git/espresso_git/build/Espresso \$HOME/git/sim_scripts/Sim_graphite_walls_explicit_oneLayer.tcl $sc \$HOME/data/graphite_CCexplicit_oneLayer/${run} &"
	}
	puts $o "wait"
	puts $o "exit 0"
	close $o
