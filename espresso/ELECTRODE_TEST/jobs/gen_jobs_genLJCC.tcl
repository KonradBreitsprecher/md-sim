...
set pot_max 6.0
set num_pots 10.0
set path "$::env(HOME)/git/sim_scripts/jobs/pbsjobs_genlj_cc01"
puts $path
for {set run 1} { $run <= $runs } {incr run} {
	...
	foreach sc $schargeList {
		puts $o "aprun -n 4 -N 4 \$HOME/git/espresso_git/build/Espresso \$HOME/git/sim_scripts/Sim_genLJ_walls_constCharge.tcl $sc \$HOME/data/genLJ_CC/${run} \$HOME/data/genLJ_CC/${run}/checkpoint_scharge_${sc}.dat &"
#		puts $o "aprun -n 4 -N 4 \$HOME/git/espresso_git/build/Espresso \$HOME/git/sim_scripts/Sim_genLJ_walls_constCharge.tcl $sc \$HOME/data/genLJ_CC/${run} &"
	}
	puts $o "wait"
	puts $o "exit 0"
	close $o
