...
set pot_max 6.0
set num_pots 10.0
set path "$::env(HOME)/git/sim_scripts/jobs/pbsjobs_genlj_cc_mbc"
puts $path
for {set run 1} { $run <= $runs } {incr run} {
	set pbsfname "$path/job_gen_run${run}.pbs"
	set o [open $pbsfname "w"]
	puts $o "#!/bin/bash"
	puts $o "#PBS -N BMIMPF6_GenLJ_CC_run${run}"
	puts $o "#PBS -l mppwidth=[expr int(4*${num_pots})]"
	puts $o "#PBS -l mppnppn=4"
	puts $o "#PBS -l walltime=24:00:00"
	foreach sc $schargeList {
#		puts $o "aprun -n 4 -N 4 \$HOME/git/espresso_git/build/Espresso \$HOME/git/sim_scripts/Sim_genLJ_walls_constCharge_MBC.tcl $sc \$HOME/data/genLJ_CC_MBC/${run} \$HOME/data/genLJ_CC_MBC/${run}/checkpoint_scharge_${sc}.dat &"
		puts $o "aprun -n 4 -N 4 \$HOME/git/espresso_git/build/Espresso \$HOME/git/sim_scripts/Sim_genLJ_walls_constChargeMetallicBCs.tcl $sc \$HOME/data/genLJ_CC_MBC/${run} &"
	}
	puts $o "wait"
	puts $o "exit 0"
	close $o
