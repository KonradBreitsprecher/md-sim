...
set partBorderL 0
#...till...
set partBorderR $box_l

set tot_box_l [expr $gap+$box_l]
...
dielectric_hexagonal_wall dist [expr -$graphenePPDist] normal 0 0 1.0 shift [expr $CCbondlength*0.5*sqrt(3.0)] [expr $CCbondlength * 0.5+$sy] lattice_param $CCbondlength sigma 0 eps 10000.0 type $icc_wall_type icc 0
dielectric_hexagonal_wall dist [expr -$graphenePPDist2] normal 0 0 1.0 shift 0 $sy lattice_param $CCbondlength sigma 0 eps 10000.0 type $icc_wall_type icc 0


dielectric_hexagonal_wall dist [expr -$box_l] normal 0 0 -1.0 shift 0 $sy lattice_param $CCbondlength sigma 0 eps 10000.0 type $icc_wall_type icc 0
dielectric_hexagonal_wall dist [expr -$box_l-$graphenePPDist] normal 0 0 -1.0 shift [expr $CCbondlength*0.5*sqrt(3.0)] [expr $CCbondlength * 0.5+$sy] lattice_param $CCbondlength sigma 0 eps 10000.0 type $icc_wall_type icc 0
dielectric_hexagonal_wall dist [expr -$box_l-$graphenePPDist2] normal 0 0 -1.0 shift 0 $sy lattice_param $CCbondlength sigma 0 eps 10000.0 type $icc_wall_type icc 0
...
for {set i $startIndexParts} { $i < [expr $n_ion + $startIndexParts] } {incr i} {
    ...
    set posz [expr ($box_l-($lj_sig_w+$lj_sig_a))*[t_random]+ ($lj_sig_w+$lj_sig_a)*0.5]
...
for {set j $i} { $j < [expr 5*$n_ion + $startIndexParts] } {incr j 4} {
    set posx [expr $box_x*[t_random]]
    set posy [expr $box_y*[t_random]]
    set posz [expr ($box_l-($lj_sig_w+$lj_sig_c3*2.0))*[t_random]+ ($lj_sig_w+$lj_sig_c3*2.0)*0.5]
...
puts [inter coulomb $l_b p3m tunev2 accuracy 1e-2]
inter coulomb elc 1e-4 $gap capacitor $UBat
...
set obs_scharge [open "$path/s_charge_${UBat_V}" "w"]
