...
set Ez_ext [expr $UBat/$box_z_tot]

set infile [open "$pathToCDC" "r"]
...
set iccParticles [llength $cdc_coords]
set iccParticlesLeft [expr int($iccParticles/2)]
...
for {set i 0} {$i < $iccParticles} {incr i} {
    #TODO: CALC CORRECT NORMALS
    set nx 0.0
    set ny 0.0 
    if {$i < [expr $iccParticles/2]} {
        set nz 1.0
    } else {
        set nz -1.0
    }
...
#writevsf $f 
#close $f
#prepare_vmd_connection vmdout start wait 4
...
if {$useCheckpoint == "no"} {
	...
	inter $c3_type $wall_type lennard-jones $lj_eps_mix_c3_w $lj_sig_mix_c3_w [expr 2.0*$lj_sig_mix_c3_w] 0.004 0 0
	inter $c1_type $wall_type lennard-jones $lj_eps_mix_c1_w $lj_sig_mix_c1_w [expr 2.0*$lj_sig_mix_c1_w] 0.004 0 0
	inter $c2_type $wall_type lennard-jones $lj_eps_mix_c2_w $lj_sig_mix_c2_w [expr 2.0*$lj_sig_mix_c2_w] 0.004 0 0
	inter $a_type $wall_type lennard-jones $lj_eps_mix_a_w $lj_sig_mix_a_w [expr 2.0*$lj_sig_mix_a_w] 0.004 0 0 
	...
	while {1} {
		...
		set E [analyze energy total]
			set dE [expr $E-$E_o]
		set E_o $E
		if {[expr abs($dE)] < 10000} {
		    if {$size < 1} {
			set size [expr $size + 0.1]
			if {$size > 1} {set size 1}
		    } else {
			set cut [expr $cut - 0.005]
			if {$cut < 0} {break}
		    }
		}
...
if {$useCheckpoint == "yes"} {
	...
	for {set temp_eq_loop 0} {$temp_eq_loop<20} {incr temp_eq_loop} {
		...
		set num_Aions_in_cdcL 0
		set num_Aions_in_cdcR 0
		set num_Aions_in_bulk 0
		set num_Cions_in_cdcL 0
		set num_Cions_in_cdcR 0
		set num_Cions_in_bulk 0
		foreach i $anionList {
			set z [lindex [part $i print pos] 2] 
			if {$z > [expr $gapL + $box_z - $cdc_w]} {
				incr num_Aions_in_cdcR
			} elseif { $z > [expr $gapL + $cdc_w] } {
				incr num_Aions_in_bulk
			} else {
				incr num_Aions_in_cdcL
			}
		}
		foreach i $cationComList {
			set z [lindex [part $i print pos] 2] 
			if {$z > [expr $gapL + $box_z - $cdc_w]} {
				incr num_Cions_in_cdcR
			} elseif { $z > [expr $gapL + $cdc_w] } {
				incr num_Cions_in_bulk
			} else {
				incr num_Cions_in_cdcL
			}
		}
		puts "Anion cnts:  Left CDC: $num_Aions_in_cdcL   Bulk: $num_Aions_in_bulk   Right CDC: $num_Aions_in_cdcR"
		puts "Cation cnts: Left CDC: $num_Cions_in_cdcL   Bulk: $num_Cions_in_bulk   Right CDC: $num_Cions_in_cdcR"
		puts "Total cnts:  Left CDC: [expr $num_Cions_in_cdcL+$num_Aions_in_cdcL]   Bulk: [expr $num_Cions_in_bulk+$num_Aions_in_bulk]   Right CDC: [expr $num_Cions_in_cdcR+$num_Aions_in_cdcL]"
	}
...
#-----------------------
puts "\n--> Tune p3m / Activate ICC / Apply field"
#-----------------------
...
puts [inter coulomb $l_b p3m tunev2 accuracy 1e-4 mesh 64]

#iccp3m $iccParticles epsilons $icc_epsilons normals $icc_normals areas $icc_areas sigmas $icc_sigmas ext_field 0.0 0.0 $Ez_ext eps_out 1.0 relax 0.7 max_iterations 100 convergence 1e-4

#APPLY E-FORCE ON IONS
for {set i $iccParticles} { $i < [expr $n_ion + $iccParticles] } {incr i} {
...
#	}	
	...
	puts $obs_energy [analyze energy]c
