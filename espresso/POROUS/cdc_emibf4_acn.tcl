...
for {set i 0} {$i < [expr $iccParticles/2]} {incr i} {
    ...
    part $i pos [expr $au_to_angs * [lindex $c 0]] [expr $au_to_angs * [lindex $c 1]] [expr $au_to_angs * [lindex $c 2]] 
}

#RIGHT CDC BORDER
set cdcShiftRight [expr $box_z - $au_to_angs * $cdc_maxz] 
...
for {set i $iccParticles} { $i < [expr $n_ion + $iccParticles] } {incr i} {
    ...
    set posz [expr ($box_z-2*$partMargin)*[t_random]+$partMargin]
...
set obs_schargeL [open "$path/s_charge_t_sumL.dat" "a"]
set obs_schargeR [open "$path/s_charge_t_sumR.dat" "a"]
...
while {1} {
	...
	puts $obs_schargeL "$md_time_ns [expr $iccChargeLeft]";
	puts $obs_schargeR "$md_time_ns [expr $iccChargeRight]";
